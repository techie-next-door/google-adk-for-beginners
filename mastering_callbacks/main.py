...
async def run_agent(session_id:str, query:str):
    ...
    state_delta = {}
    if len(query) <= 10 and (query or "").strip().lower() in {"hi", "hello", "hey"}:
        state_delta = {"only_hi" : True}
    else:
        state_delta = {"only_hi": False}

    async for event in runner.run_async(user_id="example_user",state_delta=state_delta,session_id=session_id,new_message=types.Content
